...
impl TriangleMesh {
    pub fn new(obj: Obj, material: Material, transformation: Transformation) -> Self {
        let min_x = obj.vertexes.iter().min_by(|v1, v2| v1.x.partial_cmp(&v2.x).unwrap()).unwrap().x;
        let min_y = obj.vertexes.iter().min_by(|v1, v2| v1.y.partial_cmp(&v2.y).unwrap()).unwrap().y;
        let min_z = obj.vertexes.iter().min_by(|v1, v2| v1.z.partial_cmp(&v2.z).unwrap()).unwrap().z;

        let max_x = obj.vertexes.iter().max_by(|v1, v2| v1.x.partial_cmp(&v2.x).unwrap()).unwrap().x;
        let max_y = obj.vertexes.iter().max_by(|v1, v2| v1.y.partial_cmp(&v2.y).unwrap()).unwrap().y;
        let max_z = obj.vertexes.iter().max_by(|v1, v2| v1.z.partial_cmp(&v2.z).unwrap()).unwrap().z;
...
impl Shape for TriangleMesh {
    ...
    fn hit(&self, ray: &Ray) -> bool {
        ...
        self.aabb.hit(&inv_ray) && self.triangles.iter().any(|triangle| triangle.intersect(&inv_ray).is_some())
    }
}
